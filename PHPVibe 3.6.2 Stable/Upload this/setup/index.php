...
echo 'To continue: <strong>edit database and site details</strong> in file vibe_config.php <br />';
die();
}	
//Config include
...
if(!is_null($db->errors)) {
foreach ($db->errors as $arg) {
echo "<section class=\"panel panel-danger panel2x\">
...
<a href=\"http://store.phprevolution.com/licenses\" target=\"_blank\">Browse to : My Licenses</a> <br />
...
$error = 0;
if(is_licensed()) {
echo '<div class="msg-win">License key is not empty</div>';
}
...
echo '<div class="msg-win">Thumbs folder ('.ABSPATH.'/cache/thumbs) is writeable</div>';
}
...
if (phpversion() < 5.3) {
echo '<div class="msg-hint">Recommended php version is 5.3 (and above)  while your version is '.phpversion().'! However, this is optional.</div>';
} 
if($error > 0) {
echo "<section class=\"panel panel-danger\">
<div class=\"panel-heading\">Some things require attention</div> 
<div style=\"padding:30px;\">Please correct the ".$error." errors listed above to continue (red-ish background). </div></section>";
die();
} else {
...
<div class="panel-heading">Administrator creation</div>
...
<div class="panel-heading">Setup is done</div><div style="padding:15px;"><a href="'.$site_url.'/login/"><strong>Login to your website</strong> here</a> with the administrator account then head to <a href="'.str_replace("setup",ADMINCP,$base_href).'">/'.ADMINCP.'</a> for the admin panel. <br> Thank you for choosing PHPVibe!';
echo '<a class="btn btn-large btn-primary pull-right" href="'.$site_url.'/login/">Setup is complete. Login</a></div></section>';
}
echo '
</div>
</div>
