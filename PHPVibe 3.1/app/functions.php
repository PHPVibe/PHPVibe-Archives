<?php  function w_icons($name) {
 global $site_url;
 $white_icons = $site_url."tpl/images/icons/white/";
 return  $white_icons.$name.".png";
 }
 
function theme($folder=null) {
 global $site_url;
 if(is_null($folder)){
 return   $site_url."tpl/"; 
 } else {
  return $site_url."tpl/".$folder."/"; 
 }
 }
...
function is_owner($current){
global $user;
if(!$user->isAuthorized()) {
return false;
} elseif($user->getId() == $current){
return true;
} else {
return false;
}
...
function strip_common($string){
	...
	$good = array_diff($pieces, $commonWords);
	return $good;
}
