...
$target_path = $config->site->mediafolder."/".$config->site->videofolder."/";
$allowedExts = array();
$maxFileSize = 0;
...
$inserttmp = dbquery("INSERT INTO videos_tmp (`uid`, `name`, `path`, `ext`) VALUES ('".$usr."', '".$name."', '".$file."', '".$ext."')");
}
...
if ($headers['X-Requested-With']=='XMLHttpRequest') { 
	...
	if (in_array($ext,$allowedExts) or empty($allowedExts)) {
		...
		if ($output!=false) {
			...
			$insertit = $extrastamp.str_replace(" ","-",$fileName);
			$fakefile = $fileName;
			vinsert($insertit,$fakefile);
