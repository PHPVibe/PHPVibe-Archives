...
class pagination{
	...
	*/
	public function show_pages($pagi_url){
		$output = '<ul class="paginatia paginatiaD paginatiaD06">';
		$prev = $this->current-1;
		
		if($this->show_first==true){
			$output .= '<li> <a class="first" href="'.$pagi_url.'1">First Page</a><li>';
		}
		if($this->current>2){
			$output .= '<li><a class="previous" href="'.$pagi_url.$prev.'">Previous</a></li>';
		}
		$test = $this->pages_array();
		foreach($test as $t){
			if($this->current==$t){
				$output .= '<li><a class="current" href="'.$pagi_url.$t.'">'.$t.'</a></li>';
			} else {
		...
		if($this->current<$this->page_numbers()){
			$output .= '<li><a class="nextpage" href="'.$pagi_url.$next.'">Next</a></li>';
		}
		if($this->show_last==true){
			$output .= '<li><a class="lastpage" href="'.$pagi_url.$this->page_numbers().'">Last Page</a></li>';
		}	
