...
if( $user->isAuthorized() )
{
	...
	elseif( $message = MK_Request::getQuery('message') )
	{
		...
		else
		{
			...
			{
				$output .= '<a href="messages.php?method=compose&member='.$message->getSender().'&reply='.$message->getId().'" class="btn">Reply</a>';
			}
			$output .= '<a rel="confirm" title="Are you sure you want to delete this message?" href="messages.php?method=delete&message='.$message->getId().'" class="btn">Delete</a>';
			$output .= '<br/><h2>Messages / '.$message->getSubject().'</h2>';
	...
	elseif( ( $folder = MK_Request::getQuery('folder'))  && in_array($folder, $message_folders) )
	{
		...
		if( count($messages) > 0 )
		{
			...
			{
				$output.= '
								<br /><input type="submit" name="message_mark-as-read" value="Mark as Read" class="button black icon speechbubble">
						';
			}
			$output.= '
							<input type="submit" rel="confirm" name="message_delete" title="Are you sure you want to delete these messages?" value="Delete" class="button black icon delete">
					';
	...
	{
		$output .= '<a href="messages.php?folder=inbox" class="button blue icon mailclosed">Go to your Inbox </a>
';
