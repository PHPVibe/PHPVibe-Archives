...
$BrowsePerPage = 20;
$nr_query = "SELECT COUNT(*) FROM videos WHERE user_id ='".$router->fragment(1)."'";
...
$vbox_result=mysql_query("select * from videos WHERE user_id ='".$router->fragment(1)."' ORDER BY id DESC $limit");
$u_canonical = $site_url.'user/'.$user_profile->getId().'/'.seo_clean_url($user_profile->getDisplayName()) .'/';
$h1 = $user_profile->getDisplayName()."'s ".$lang['videos']." | <a href=\"".$u_canonical."\">more... </a>";
