...
class pagination{
	...
	*/
	public function show_pages($pagi_url){
		$output = '<div class="pagination">	<ul class="pages">
';
		$prev = $this->current-1;
		
		if($this->show_first==true){
			$output .= '<li class="prev"> <a href="'.$pagi_url.'1">First Page</a><li>';
		}
		if($this->current>2){
			$output .= '<li><a href="'.$pagi_url.$prev.'">Previous</a></li>';
		}
		$test = $this->pages_array();
		foreach($test as $t){
			if($this->current==$t){
				$output .= '<li><a class="active" href="'.$pagi_url.$t.'">'.$t.'</a></li>';
			} else {
		...
		if($this->show_last==true){
			$output .= '<li><a href="'.$pagi_url.$this->page_numbers().'">Last Page</a></li>';
		}	
