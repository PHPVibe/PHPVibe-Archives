...
global $user, $lang, $CCOUNT , $SHOWNAME, $SHOWMAIL, $ALLOWLIKE;
...
ORDER BY  em_comments.id limit 0,10000");
	...
	while($comment= mysql_fetch_array($comments)) {
        ...
        $html .= '<div class="emComment emComment_'.$object_id.' '.($counter < ($total - ($CCOUNT - 1))?'emHiddenComment emHiddenComment_'.$object_id:'').'" id="comment_'.$comment['id'].'" '.($counter < ($total - ($CCOUNT - 1))?'style="display:none"':'').'>
				  ...
				  $html .= ' <a href="'.$site_url.'user/'.$comment['sender_id'].'/'.$comment['display_name'].'"><img src="'.$site_url.'com/timthumb.php?src='.$comment['avatar'].'&h=32&w=32&crop&q=100" width="32" height="32" alt="'.$comment['display_name'].'" />
                    </a></div>';
    ...
    $html .= '<div id="emAddComment_'.$object_id.'" class="emAddComment">
                <form method="post" action="'.$site_url.'components/addComment.php" onsubmit="return false;">
                    ...
                    <input type="hidden" name="object_id" value="'.$object_id.'" />
                        ...
                        <input type="submit" id="emAddButton_'.$object_id.'" value="'.$lang['comment'].'" onclick="addEMComment(\''.$object_id.'\')" />
