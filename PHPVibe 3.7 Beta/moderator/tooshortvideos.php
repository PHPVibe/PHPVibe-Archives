...
$count = $db->get_row("Select count(*) as nr from ".DB_PREFIX."videos where pub > 0 and featured > 0 ");
...
/*$videos = $db->get_results("select id,title,thumb, views, liked,featured, duration from ".DB_PREFIX."videos where duration <= 180 and featured > 0 ORDER BY ".DB_PREFIX."videos.id DESC ".this_limit()."");*/

} else {
$count = $db->get_row("Select count(*) as nr from ".DB_PREFIX."videos where pub > 0 ");
/*$d = (isset($_GET['seconds'])) ?  $_GET['seconds'] : 180;*/
$d = (isset($_REQUEST['seconds'])) ?  $_REQUEST['seconds'] : 180;
$videos = $db->get_results("select id,title,thumb, views, liked,featured, duration from ".DB_PREFIX."videos where pub > 0 and duration <= ".$d." ORDER BY ".DB_PREFIX."videos.id DESC ".this_limit()."");
/*$videos = $db->get_results("select id,title,thumb, views, liked,featured, duration from ".DB_PREFIX."videos where duration <= 180 ORDER BY ".DB_PREFIX."videos.id DESC ".this_limit()."");*/

}
?>

<div class="row-fluid">
		    	<form class="search widget" action="" method="get" onsubmit="location.href='<?php echo admin_url('tooshortvideos'); ?>&seconds=' + encodeURIComponent(this.key.value); return false;">
		    		<div class="autocomplete-append">			   
			    		<input type="text" name="key" placeholder="Seconds (sorting)" id="key" />
...
<h3>Video management (Find Too Short Videos)</h3>	
...
/*if(isset($_GET['sort']) ) {
$ps = admin_url('tooshortvideos').'&sort='.$_GET['sort'].'&p=';
} else {
$ps = admin_url('tooshortvideos').'&p=';
}
*/
