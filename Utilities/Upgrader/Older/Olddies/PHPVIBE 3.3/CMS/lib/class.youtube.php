...
$content = $this->getDataFromUrl($feedURL);
			...
			for($i=0; $i<count($videosList); $i++) {
				...
				$videosDatas['videos'][$i]['title'] = htmlentities($videosList[$i]['title'], ENT_QUOTES, "UTF-8");
				$videosDatas['videos'][$i]['description'] = htmlentities($videosList[$i]['description'], ENT_QUOTES, "UTF-8");
...
function youtube_import($video=array(), $cat = null) {
global $db;
if(isset($video["videoid"]) && isset($video["title"]) ) {
$video["path"] = 'http://www.youtube.com/watch?v='.$video["videoid"];
$video["thumbnail"] = str_replace('default.jpg','0.jpg', $video["thumbnail"]);
...
('".intval(get_option('videos-initial'))."','".$video["path"]."', '".get_option('importuser')."', now() , '".$video["thumbnail"]."', '".toDb($video["title"]) ."', '".intval($video["duration"])."', '".toDb(_post('tags'))."', '0', '0','".toDb($cat)."','".toDb($video["description"])."','0')");	
//var_dump($video);
} else {
