...
if($video && $playlist) {
$did["what"] = _lang("collected a video in playlist ").' <a class="text-primary" href="'.playlist_url($playlist ->id , $playlist ->title).'" title="'.stripslashes($playlist ->title).'"><i class="icon-list-alt"></i> <strong>'.stripslashes(_cut($playlist ->title, 146)).'</strong></a>';
...
if($video) {
$did["what"] = _lang("subscribed to ") .' <a class="text-primary" href="'.profile_url($video->id , $video->name).'" title="'.stripslashes($video->name).'">'.stripslashes(_cut($video->name, 46)).'</a>';
...
if($ad){
return '
<div class="floating-video-ad adx '.$ad->ad_pos.'">'._html($ad->ad_content).'<i class="icon-off close-ad"></i></div>
';
}
} else {
