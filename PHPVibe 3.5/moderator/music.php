...
$count = $db->get_row("Select count(*) as nr from ".DB_PREFIX."videos where pub > 0 and media ='2' and featured > 0 ");
$videos = $db->get_results("select id,title,thumb, views, liked,featured, duration from ".DB_PREFIX."videos where pub > 0 and media ='2' and featured > 0 ORDER BY ".DB_PREFIX."videos.id DESC ".this_limit()."");

} else {
$count = $db->get_row("Select count(*) as nr from ".DB_PREFIX."videos where pub > 0 and media ='2' ");
$videos = $db->get_results("select id,title,thumb, views, liked,featured, duration from ".DB_PREFIX."videos where pub > 0 and media ='2' ORDER BY ".DB_PREFIX."videos.id DESC ".this_limit()."");
}
?>
<div class="row-fluid">
...
if($videos) {
if(isset($_GET['sort']) ) {
$ps = admin_url('music').'&sort='.$_GET['sort'].'&p=';
} else {
$ps = admin_url('music').'&p=';
}
$a = new pagination;	
                        ...
                        <table class="table table-bordered table-checks">
						  ...
						  <?php foreach ($videos as $video) { ?>
                              <tr>
								  ...
								  <td>
								  <p><a href="<?php echo admin_url('music');?>&p=<?php echo this_page();?>&delete-video=<?php echo $video->id;?>"><i class="icon-trash" style="margin-right:5px;"></i><?php echo _lang("Unpublish"); ?></a></p>
								  ...
								  <p><a href="<?php echo admin_url('music');?>&p=<?php echo this_page();?>&feature-video=<?php echo $video->id;?>"><i class="icon-star" style="margin-right:5px;"></i><?php echo _lang("Feature"); ?></a></p>
								 <?php } else { ?>
								  <p><a href="<?php echo admin_url('music');?>&p=<?php echo this_page();?>&feature-video=<?php echo $video->id;?>"><i class="icon-remove-sign" style="margin-right:5px;"></i><?php echo _lang("Unfeature"); ?></a></p>
								 <?php } ?>
