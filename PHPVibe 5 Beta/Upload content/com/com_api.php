...
if($continue){
$notif = $db->get_row("Select count(*) as nr from vibe_activity where ((type not in (8,9) and vibe_activity.object in (select id from vibe_videos where user_id ='".user_id()."' ) ) or (type in (8,9) and vibe_activity.object in (select id from vibe_images where user_id ='".user_id()."' ) ) and user <> '".user_id()."') and `date` > '".user_noty()."'");
if($notif) {
$count["buzz"] = $notif->nr;	
}
