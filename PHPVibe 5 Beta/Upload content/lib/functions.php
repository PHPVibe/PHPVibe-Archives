...
if($image) {
if($image->embed || $image->remote) {
//delete imediatly if remote
$db->query("DELETE from ".DB_PREFIX."images where id='".intval($id)."'");
} else {
//try to delete file to
$vid = new Vibe_Providers(10, 10);
$source = $vid->VideoProvider($image->source);
if(($source == "localimage") || ($source == "localfile") ) {
$path = ABSPATH.'/'.get_option('mediafolder').str_replace(array("localimage", "localfile"),array("", ""),$image->source);
//remove image file
remove_file($path);
$thumb = $image->thumb;
if($thumb && ($thumb != "uploads/noimage.png")&& ($thumb != "media/thumbs/xmp3.jpg")) {
...
$db->query("DELETE from ".DB_PREFIX."images where id='".intval($id)."'");
$db->query("DELETE from ".DB_PREFIX."hearts where vid='".intval($id)."'"); 
$db->query("DELETE from ".DB_PREFIX."em_comments where object_id='img".intval($id)."'");
