...
require_once("apifunctions.php");
	...
	$default_tag = $overall_tag;
 

if (filterBadWords($overall_tag,"badwords.txt") != 0) {
...
$random_tag_pagination = '';
	...
	if($pages>7) {
		if($currentpage>4) {
			$random_tag_pagination .= '<a href="'.$site_url.'show/'.$newtag.'/page-1">First</a>&nbsp;';
		}
	}
					
	if($currentpage > 1) {
		$random_tag_pagination .= '&nbsp;<a href="'.$site_url.'show/'.$newtag.'/page-'.$pageless.'">Prev</a>&nbsp;';
	}
	...
	for($i=0;$i<$pages;$i++) {
		if($i+1>=$startpage && $i+1<=$endpage) {
			...
			else { $random_tag_pagination .= '<a href="'.$site_url.'show/'.$newtag.'/page-'.$next.'">'.$next.'</a>'; }
				
		}
	}
	
	if($currentpage!=$endpage && $currentpage<=85) {
		$random_tag_pagination .= '&nbsp;<a href="'.$site_url.'show/'.$newtag.'/page-'.$pagemore.'">Next</a>&nbsp;';
	}
	
	if($pages>7) {
		if($currentpage<$pages-3 && $currentpage<=85) {
			$random_tag_pagination .= '&nbsp;<a href="'.$site_url.'show/'.$newtag.'/page-'.$pages.'">Last</a>&nbsp;';
		}
	}
...
$standard_feed_pagination = '';
	...
	if($pages>7) {
		if($currentpage>4) {
			$standard_feed_pagination .= '<a href="'.$site_url.'show/'.$newtag.'/page-1">First</a>&nbsp;';
		}
	}
					
	if($currentpage > 1) {
		$standard_feed_pagination .= '&nbsp;<a href="'.$site_url.'show/'.$newtag.'/page-'.$pageless.'">Prev</a>&nbsp;';
	}
	...
	for($i=0;$i<$pages;$i++) {
		if($i+1>=$startpage && $i+1<=$endpage) {
			...
			else { $standard_feed_pagination .= '<a href="'.$site_url.'show/'.$newtag.'/page-'.$next.'">'.$next.'</a>'; }
				
		}
	}
	
	if($currentpage!=$endpage) {
		$standard_feed_pagination .= '&nbsp;<a href="'.$site_url.'show/'.$newtag.'/page-'.$pagemore.'">Next</a>&nbsp;';
	}
	
	if($pages>7) {
		if($currentpage<$pages-3) {
			$standard_feed_pagination .= '&nbsp;<a href="'.$site_url.'show/'.$newtag.'/page-'.$pages.'">Last</a>&nbsp;';
		}
	}
...
// Show Template
if($template != "") {
	if(is_file('templates/'.$template.'/tags.php')) {
		require_once('templates/'.$template.'/tags.php');
	} else {
