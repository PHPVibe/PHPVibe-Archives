...
if (isset($_POST['deleteBtn'])) {
    foreach ($_POST as $key => $value) {
		...
		if ($key != "deleteBtn") {
		   $sql = dbquery("UPDATE private_messages SET recipientDelete='1', opened='1' WHERE id='$value' AND to_id='$my_id' LIMIT 1");
...
$sql = dbquery("SELECT * FROM private_messages WHERE to_id='$my_id' AND recipientDelete='0' ORDER BY id DESC LIMIT 100");
...
while($row = mysql_fetch_array($sql)){ 
    ...
    $ret = dbquery("SELECT id, username FROM myMembers WHERE id='$fr_id' LIMIT 1");
