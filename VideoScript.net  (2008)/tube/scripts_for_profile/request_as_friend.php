...
if ($_POST["request"] == "requestFriendship") {
	...
	$sql_frnd_arry_mem1 = dbquery("SELECT friend_array FROM myMembers WHERE id='$mem1' LIMIT 1"); 
	...
	$sql = dbquery("SELECT id FROM friends_requests WHERE mem1='$mem1' AND mem2='$mem2' Limit 1");
	...
	$sql = dbquery("SELECT id FROM friends_requests WHERE mem1='$mem2' AND mem2='$mem1' Limit 1");
	$numRows = mysql_num_rows($sql);
	if ($numRows > 0) {
	...
	$sql = dbquery("INSERT INTO friends_requests (mem1, mem2, timedate) VALUES('$mem1','$mem2',now())") or die (mysql_error("Friend Request Insertion Error"));
	//$sql = dbquery("INSERT INTO pms (to, from, time, sub, msg) VALUES('$mem2','XXXXX',now(),'New Friend Request','You have a new Friend Request waiting for approval.<br /><br />Navigate to your profile and check your friend requests. <br /><br />Thank you.')") or die (mysql_error("Friend Request PM Insertion Error"));
    //$id = mysql_insert_id();
...
if ($_POST["request"] == "acceptFriend") {
	...
	$sql_frnd_arry_mem1 = dbquery("SELECT friend_array FROM myMembers WHERE id='$mem1' LIMIT 1"); 
	$sql_frnd_arry_mem2 = dbquery("SELECT friend_array FROM myMembers WHERE id='$mem2' LIMIT 1"); 
	while($row=mysql_fetch_array($sql_frnd_arry_mem1)) { $frnd_arry_mem1 = $row["friend_array"]; }
    ...
    $UpdateArrayMem1 = dbquery("UPDATE myMembers SET friend_array='$frnd_arry_mem1' WHERE id='$mem1'") or die (mysql_error());
    $UpdateArrayMem2 = dbquery("UPDATE myMembers SET friend_array='$frnd_arry_mem2' WHERE id='$mem2'") or die (mysql_error());
	$deleteThisPendingRequest = dbquery("DELETE FROM friends_requests WHERE id='$reqID' LIMIT 1"); 
...
if ($_POST["request"] == "removeFriendship") {
	...
	$sql = dbquery("UPDATE myMembers SET friend_array='$newStringForMem1' WHERE id='$mem1'");
	$sql = dbquery("UPDATE myMembers SET friend_array='$newStringForMem2' WHERE id='$mem2'");
