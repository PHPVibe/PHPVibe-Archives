...
$user_module = MK_RecordModuleManager::getFromType('user');
	...
	$u_url = $site_url.'user/'.$current_user->getId().'/'.seo_clean_url($current_user->getDisplayName()) .'/';
		...
		{
		$userlistnew.= '';
			$userlistnew.= '<a href="'.$u_url.'" title = "'.$current_user->getDisplayName().'"><img class="border_white" src="'.$site_url.'components/thumb.php?f='.$avatar.'&h=70&w=70&m=crop" alt = "'.$current_user->getDisplayName().'"/></a>';
		$userlistnew.= '';
		}
