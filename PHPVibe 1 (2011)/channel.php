...
$cid = preg_replace('#[^0-9]#i', '', $_GET['id']);
$csql = dbquery("SELECT cat_name FROM `channels` WHERE `cat_id` = ".$cid." LIMIT 0,1");
while($row = mysql_fetch_array($csql)){
$category =  $row["cat_name"];
$catdesc =  $row["cat_desc"];

}
$sql = dbquery("SELECT id FROM videos WHERE category = '".$cid."' ORDER BY id DESC"); 
$nr = mysql_num_rows($sql);
if ($nr == 0) {
$url = $site_url.'?nochanell=1';
redirect($url); 
exit();
}
if (isset($_GET['pn'])) { 
...
$head_title = array();
$head_desc = array();
$head_title[] = 'Browsing the '.$category.' channel '.$pn.' of '.$lastPage; 
$head_desc[] = $category.':'.$catdesc.'. Curent Page it\'s '.$pn; 
...
<div class="col col9">
  <div class="col-bkg clearfix">
    <h1><?php echo __("Video channel:");?> <?php echo $category.' ('.$nr;?> <?php echo __(" videos");?>)</h1>

<div class="vibe_contain">
 <ul>
<?php 
$limit = 'LIMIT ' .($pn - 1) * $BrowsePerPage .',' .$BrowsePerPage; 
 $sql = dbquery("SELECT * FROM videos WHERE category = '".$cid."' ORDER BY views DESC $limit");
...
<?php
   ...
   $url = $config->site->url.'category/'.$cid.'/'.$category.'/page-';
